...
def create_minimap(_map):
    minimap = [[_map[i][j].copy() for j in range(len(_map[0]))] for i in range(len(_map))]
    for i in range(len(minimap[0])):
        for j in range(len(minimap)):
            if _map[j][i] == wall:
                minimap[j][i][0] = create_sprite(os.path.join("texture", "wall.png"), 3)
            else:
                minimap[j][i][0] = create_sprite(os.path.join("texture", "Ground_1.png"), 3)
                
    hero_marker = create_sprite(os.path.join("texture", "hero_marker.png"), 3)
    
    return minimap, hero_marker
...
def reload_game(engine, hero):
    ...
    minimap, hero_marker = create_minimap(_map)
    engine.load_map(_map)
    engine.load_minimap(minimap, hero_marker)
...
def apply_blessing(engine, hero):
    if hero.gold >= int(20 * 1.5**engine.level) - 2 * hero.get_stats()["intelligence"]:
        engine.score += 0.2
        hero.gold -= int(20 * 1.5**engine.level) - \
            2 * hero.get_stats()["intelligence"]
...
def remove_effect(engine, hero):
    ...
    if hero.gold >= int(10 * 1.5**engine.level) - 2 * hero.get_stats()["intelligence"] and "base" in dir(hero):
        hero.gold -= int(10 * 1.5**engine.level) - \
            2 * hero.get_stats()["intelligence"]
        if isinstance(engine.hero, Objects.Darkness):
            floor1[0] = create_sprite(os.path.join("texture", "Ground_1.png"), engine.sprite_size)
            floor2[0] = create_sprite(os.path.join("texture", "Ground_2.png"), engine.sprite_size)
            floor3[0] = create_sprite(os.path.join("texture", "Ground_3.png"), engine.sprite_size)
...
def add_gold(engine, hero):
    ...
    elif random.randint(1, 10) in range(3):
        engine.score -= 0.05
        engine.hero = Objects.Darkness(hero)
        floor1[0] = floor4[0]
        floor2[0] = floor4[0]
        floor3[0] = floor4[0]
        engine.notify("Fear darkness!")
    else:
        engine.score += 0.1
...
def upgrade_equipment(engine, hero):
    prise = 500 * (hero.item_level + 1)
    if hero.gold >= prise and hero.item_level < 3:
        hero.gold -= prise
        engine.score += 0.2
        hero.item_level += 1
        hero.sprite = create_sprite(os.path.join("texture", f"Hero{hero.item_level}.png"),
                                    engine.sprite_size)
        engine.notify(f'Equipment upgraded for {prise} gold.')
    elif hero.item_level >= 3:
        engine.notify('You have no items to upgrade.')
    else:
        engine.notify(f'You need to pay {prise} gold.')
...
def service_init(sprite_size, full=True):
    ...
    if full:
        ...
        floor4[0] = create_sprite(os.path.join("texture", "Ground_4.png"), sprite_size)
